...
async def mines_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return await query.answer("Invalid or expired Mines button.", show_alert=True)
    if query.from_user.id != uid:
        return await query.answer("Not your game.", show_alert=True)
    game = active_mines.get(uid)
    if not game:
        return await query.answer("Game expired.", show_alert=True)
    if getattr(query.message, "message_id", None) != game.get("message_id"):
        return await query.answer("Game expired.", show_alert=True)
    ...
    if lock.locked():
        return await query.answer("Processing your previous tap…")

    async with lock:
        if active_mines.get(uid) is not game:
            return await query.answer("Game expired.", show_alert=True)
        # A lower revision is a normal rapid tap from the same message. Accept
        # it when the tile remains unopened; exact revision equality rejects
        # legitimate user input while Telegram is applying the previous edit.
        if revision > game["revision"]:
            return await query.answer("Old button. Use the latest board.", show_alert=True)
        _touch_game(game)
