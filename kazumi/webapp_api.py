...
_DASHBOARD_EXECUTOR = ThreadPoolExecutor(max_workers=8, thread_name_prefix="kazumi-dashboard")
_DASHBOARD_CACHE_TTL_SECONDS = 30
_WEBAPP_DASHBOARD_CACHE = {}
_WEBAPP_DASHBOARD_CACHE_LOCK = Lock()
...
_WEBAPP_USER_PROJECTION = {
    "user_id": 1, "name": 1, "username": 1, "balance": 1, "bank": 1,
    "xp": 1, "rank": 1, "status": 1, "kills": 1, "game_wins": 1,
    "daily_streak": 1, "is_premium": 1, "premium_lifetime": 1,
    "premium_until": 1, "premium_plan": 1, "last_daily": 1,
    "last_spin": 1, "last_fortune": 1, "last_interest": 1,
    "kill_limit": 1, "rob_limit": 1, "protection_expiry": 1,
    "inventory": {"$slice": ["$inventory", 60]},
    "achievements": {"$slice": ["$achievements", 12]},
    "waifu_count": {"$size": {"$ifNull": ["$waifus", []]}},
}


def _webapp_user_doc(user_id):
    """Load the compact profile needed by the Mini App, not a player's full collection."""
    pipeline = [
        {"$match": {"user_id": int(user_id)}},
        {"$project": _WEBAPP_USER_PROJECTION},
        {"$limit": 1},
    ]
    return next(users_collection.aggregate(pipeline), None)
...
def ensure_web_user(tg_user):
    ...
    user = _webapp_user_doc(user_id)
    if user:
        changed = user.get("name") != name or user.get("username") != username
        if changed:
...
def public_user(user, include_rank=True):
    if not user:
        return None
    xp = int(user.get("xp", 0))
    level, current_xp, needed_xp = get_level(xp)
    rank = int(user.get("rank", 0) or 0)
    if include_rank:
        try:
    ...
    return {
        ...
        "waifuCount": int(user.get("waifu_count", len(waifus)) or 0),
...
_WEBAPP_LB_CACHE_LOCK = Lock()
_WEBAPP_LB_REFRESH_EXECUTOR = ThreadPoolExecutor(max_workers=1, thread_name_prefix="kazumi-leaderboard")
...
def _refresh_leaderboard():
    """Refresh rank boards away from a user-facing dashboard request."""
    try:
        eligible = leaderboard_filter()
        rich = list(users_collection.find(eligible).sort("balance", -1).limit(10).max_time_ms(1500))
        killers = list(users_collection.find(eligible).sort("kills", -1).limit(10).max_time_ms(1500))
        winners = list(users_collection.find(eligible).sort("game_wins", -1).limit(10).max_time_ms(1500))
        debt_pipeline = [
            {"$match": {"status": "active"}},
        ...
        debt = list(loans_collection.aggregate(debt_pipeline, maxTimeMS=1500))
        payload = {
        ...
        with _WEBAPP_LB_CACHE_LOCK:
            _WEBAPP_LB_CACHE["ts"] = time.time()
            _WEBAPP_LB_CACHE["payload"] = payload
    except Exception as exc:
        print(f"[WEBAPP LB REFRESH ERROR] {exc}", flush=True)
    finally:
        with _WEBAPP_LB_CACHE_LOCK:
            _WEBAPP_LB_CACHE["refreshing"] = False


def leaderboard_payload():
    """Return the latest rank snapshot immediately and refresh it in the background."""
    now = time.time()
    with _WEBAPP_LB_CACHE_LOCK:
        payload = _WEBAPP_LB_CACHE.get("payload")
        stale = not payload or (now - _WEBAPP_LB_CACHE["ts"]) >= 300
        if stale and not _WEBAPP_LB_CACHE.get("refreshing", False):
            _WEBAPP_LB_CACHE["refreshing"] = True
            _WEBAPP_LB_REFRESH_EXECUTOR.submit(_refresh_leaderboard)
        return payload or _empty_leaderboard()
...
def _cached_dashboard(user_id):
    with _WEBAPP_DASHBOARD_CACHE_LOCK:
        cached = _WEBAPP_DASHBOARD_CACHE.get(int(user_id))
        if cached and (time.monotonic() - cached["created_at"]) < _DASHBOARD_CACHE_TTL_SECONDS:
            return cached["payload"]
    return None


def _cache_dashboard(user_id, payload):
    with _WEBAPP_DASHBOARD_CACHE_LOCK:
        _WEBAPP_DASHBOARD_CACHE[int(user_id)] = {"created_at": time.monotonic(), "payload": payload}


def invalidate_dashboard_cache(user_id):
    with _WEBAPP_DASHBOARD_CACHE_LOCK:
        _WEBAPP_DASHBOARD_CACHE.pop(int(user_id), None)


def fast_dashboard_payload(user):
    user_id = int(user["user_id"])
    empty_history = {
        "recent": [],
        "summary": {
            "earned": 0, "earnedText": format_money(0),
            "spent": 0, "spentText": format_money(0),
            "net": 0, "netText": format_money(0),
            "biggestWin": 0, "biggestWinText": format_money(0),
            "biggestLoss": 0, "biggestLossText": format_money(0),
            "count": 0,
        },
    }
    leaderboard = _WEBAPP_LB_CACHE.get("payload") or {"rich": [], "killers": [], "winners": [], "debt": []}
    return {
        "ok": True,
        "partial": True,
        "user": public_user(user, include_rank=False),
        "daily": daily_state(user),
        "missions": {
            "date": datetime.utcnow().strftime("%Y-%m-%d"), "today": [], "completed": 0, "total": 0,
            "rewardReady": False, "rewardClaimed": False, "fullReward": 0, "fullXp": 0,
        },
        "cooldowns": cooldown_payload(user),
        "memory": {"count": 0, "limit": 25, "facts": []},
        "gang": {"joined": False, "top": []},
        "premium": {
            "active": premium_is_active(user), "ownerLink": OWNER_LINK or f"tg://user?id={OWNER_ID}",
            "plan": user.get("premium_plan") or ("lifetime" if user.get("premium_lifetime") else None),
            "until": user.get("premium_until").isoformat() if user.get("premium_until") else None,
            "lifetime": bool(user.get("premium_lifetime", False)), "latestPayment": None,
            "plans": [premium_plan_payload(PREMIUM_PLANS["monthly"]), premium_plan_payload(PREMIUM_PLANS["lifetime"])],
        },
        "loans": {"active": [], "pending": [], "owed": 0, "lent": 0},
...
def dashboard_payload(user, *, include_admin=False):
    user_id = int(user["user_id"])
    jobs = {
        "user": _DASHBOARD_EXECUTOR.submit(public_user, user),
        "missions": _DASHBOARD_EXECUTOR.submit(mission_payload, user_id),
        "memory": _DASHBOARD_EXECUTOR.submit(public_memory_payload, user_id),
        "gang": _DASHBOARD_EXECUTOR.submit(gang_payload, user_id),
        "premium": _DASHBOARD_EXECUTOR.submit(premium_payload, user),
        "loans": _DASHBOARD_EXECUTOR.submit(user_loans, user_id),
        "history": _DASHBOARD_EXECUTOR.submit(history_payload, user_id),
        "leaderboard": _DASHBOARD_EXECUTOR.submit(leaderboard_payload),
    }
    if include_admin:
        jobs["admin"] = _DASHBOARD_EXECUTOR.submit(admin_overview_payload, user_id)
...
def me():
    ...
    user_id = int(user["user_id"])
    is_admin = bool(admin_role_for(user_id))
    if request.args.get("fast") == "1" and not is_admin:
        cached = _cached_dashboard(user_id)
        payload = cached or fast_dashboard_payload(user)
        response = jsonify(payload)
        response.headers["X-Kazumi-Dashboard-Cache"] = "HIT" if cached else "CORE"
        response.headers["Server-Timing"] = f"dashboard;dur={(time.perf_counter() - started_at) * 1000:.1f}"
        return response

    cached = None if is_admin else _cached_dashboard(user_id)
    cache_status = "HIT" if cached else "MISS"
    payload = cached or dashboard_payload(user, include_admin=is_admin)
    if not cached and not is_admin:
        _cache_dashboard(user_id, payload)

    response = jsonify(payload)
    response.headers["X-Kazumi-Dashboard-Cache"] = cache_status
    response.headers["Server-Timing"] = f"dashboard;dur={(time.perf_counter() - started_at) * 1000:.1f}"
    return response
